/*修正1（17行目） Kp_align の定義を有効化。これでStep4で alignToLine() を解除してもコンパイルが通る。今は未使用だから「unused variable」の警告が出るかもしれないけど、エラーにはならないので無視してOK。
修正2 traceLinePI() 内に積分クランプ（±2000）を追加。Ki=0.00 の今は何も起きないけど、I制御を効かせたときに integral が暴走しなくなる。上限値は実機で要調整。
修正3 センサ読み取りを loop() の冒頭に一本化。traceLinePI() からは readLineSensors() を削除したので、Step1でもStep3でも「1ループ1回読み」で統一。判定とトレースで違う瞬間の値を使う問題が消えた。
あと、Step3解除版のコメント内に「Step4へ進むときは while(1) を消す」注意書きを足しておいた。挙動そのものは変えてないから、Step1のテストはそのまま書き込んで進められる。*/
...
// ★修正1：Step4でalignToLine()を解除したときに未定義エラーにならないよう、ここで定義を有効化
float Kp_align = 0.08;
...
// ★修正2：積分ワインドアップ対策用の上下限（Kiを効かせたとき暴走させないため）
const long INTEGRAL_MAX = 2000;
...
// PI制御によるライントレース実行関数
// ★修正3：センサ読み取りはloop()側に一本化したので、ここでは読まない（2回読み防止）
void traceLinePI() {
  ...
  // ★修正2：アンチワインドアップ（積分値が際限なく膨らむのを防ぐ）
...
void loop() {
  ...
  // (ステップ3でこのブロックのコメントアウトを解除し、下のステップ1用の2行を消します)
  
  readLineSensors(); // 毎ループ最初に1回だけセンサを読む

  if (isCrossroad() == true) {
    // 交差点を検知したらモータを停止(90)して待機
    setMotors(90, 90);
    Serial.println("Crossroad Detected! STOP.");

    // ★Step3はここで完全停止（下のwhile(1)を有効にする）。
    //   ただしStep4へ進むときは、この while(1) を消して
    //   「停止 → delay(1000) → turn90Right() → alignToLine()」に書き換えること。
    //   while(1)を残したままだと後続の関数が永遠に実行されません。
    while(1) {
  ...
  */
  ...
  // --- ステップ1（現在）の処理 ---
  // 上のブロックを有効にしたときは、以下の2行を削除（またはコメントアウト）してください
  readLineSensors(); // ★修正3：センサ読み取りはここに一本化
  traceLinePI();     // ひたすらライントレースを続ける
